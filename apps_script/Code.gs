...
//
// `accept-encoding` is stripped intentionally: UrlFetchApp transparently
// decompresses gzip/deflate but leaves Content-Encoding/Length unchanged
// in the response headers, so forwarding the browser's
// "gzip, deflate, br, zstd" preference produces a header/body mismatch
// (browser sees Content-Encoding: gzip but body is plaintext, or sees
// "br" with brotli bytes UrlFetchApp can't decode). Letting UrlFetchApp
// pick its default keeps the body and headers consistent.
const SKIP_HEADERS = {
	host: 1,
	connection: 1,
	"content-length": 1,
...
// Response headers we strip before returning. These describe a wire
// framing (compression + length) that no longer matches the bytes
// after UrlFetchApp's auto-decompression. The Go proxy recomputes
// Content-Length from the actual body length on its side.
const SKIP_RES_HEADERS = {
	"content-encoding": 1,
	"content-length": 1,
	"transfer-encoding": 1,
};
...
function _canonicalHeadersJSON(h) {
	...
	var keys = Object.keys(h).map(function (k) {
	...
	for (var i = 0; i < keys.length; i++) {
		...
		seen[keys[i]] = 1;
		parts.push(
			JSON.stringify(keys[i]) + ":" + JSON.stringify(String(lower[keys[i]])),
		);
	}
	return "{" + parts.join(",") + "}";
}
...
function _respHeaders(resp) {
	...
	try {
		raw =
			typeof resp.getAllHeaders === "function"
				? resp.getAllHeaders()
				: resp.getHeaders();
	} catch (err) {
		raw = resp.getHeaders();
	...
	for (var k in raw) {
		...
		if (SKIP_RES_HEADERS[k.toLowerCase()]) continue;
		out[k] = raw[k];
	}
	return out;
}
